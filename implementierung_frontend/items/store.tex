...
Solange kein Item ausgewählt ist, hat das Feld den Wert \code{null}.

Der Items-Reducer vereinigt \code{collection}, \code{selectedItem} und
\code{components}\footnote{auf den \code{components}-Reducer wird aufgrund
seiner Ähnlichkeit mit dem Items-Reducer nicht näher eingegangen} mit der in
\cref{ssec:gr_reducer} erläuterten Funktion \code{combineReducers}.
...
Der \code{selectedItem}-Reducer registriert Handler für diese Actions:
\code{CHANGE\_ITEM}, \code{SELECT\_ITEM\_COMPLETE},
\code{UPDATE\_ITEM\_COMPLETE}, \code{REMOVE\_ITEM\_COMPLETE}.\\
Die \code{SELECT\_ITEM\_COMPLETE}-Action aktualisiert den State mit dem im
Payload der Action übergebenen Item oder \code{null}.
