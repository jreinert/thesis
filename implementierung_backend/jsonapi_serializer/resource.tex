...
ersichtlich, eine abstrakte Klasse \code{Resource} angelegt.  Das Definieren
von Attributen und Relationen erfolgt über die Macros \code{attributes} und
\code{relationships}, die jeweils die Methoden \code{get\_attributes} und
\code{attributes(object, io)} beziehungsweise \code{get\_relationships} und
\code{relationships(object, io)} implementieren.  Die Methoden
\code{get\_attributes} und \code{get\_relationships} geben eine Hash-Map der
Attribut-/Relationsnamen mit den aktuellen Werten zurück.  Die Methoden
\code{attributes(object, io)} und \code{relationships(object, io)} dienen zur
Serialisierung der jeweiligen Daten und werden von \code{to\_cached\_json(io)}
aufgerufen.
